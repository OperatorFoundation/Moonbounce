...
class MoonbounceNetworkExtensionPacketTunnelProvider: NEPacketTunnelProvider
{
    ...
    public override func startTunnel(options: [String : NSObject]? = nil, completionHandler: @escaping (Error?) -> Void)
    {
        ...
        let connection = self.createTCPConnection(to: NWHostEndpoint(hostname: "206.189.200.18", port: "8888"), enableTLS: false, tlsParameters: nil, delegate: nil)
        connection.write("WOOOOOO".data)
        {
            maybeError in
